...
class TickerData:
    ...
    def from_binance_response(cls, data: Dict[str, Any]) -> "TickerData":
        ...
        try:
            ...
            raise ValueError(f"Invalid ticker data format: {e}") from e
    ...
    def validate(self) -> bool:
        ...
        if not (self.low_24h <= self.price <= self.high_24h):
            raise ValueError(
                f"Price {self.price} outside 24h range [{self.low_24h}, {self.high_24h}]"
            )
...
class KlineData:
    ...
    def from_binance_response(
        cls, symbol: str, interval: str, data: List[Any]
    ) -> "KlineData":
        """
        ...
        try:
            ...
            raise ValueError(f"Invalid kline data format: {e}") from e
...
class AccountInfo:
    ...
    def from_binance_response(cls, data: Dict[str, Any]) -> "AccountInfo":
        ...
        try:
            ...
            return cls(
                ...
                total_unrealized_pnl=Decimal(
                    str(data.get("totalUnrealizedProfit", "0"))
                ),
                balances=balances,
            )
        except (KeyError, ValueError, TypeError) as e:
            logger.error(f"Failed to parse account info: {e}")
            raise ValueError(f"Invalid account info format: {e}") from e
    ...
    def validate(self) -> bool:
        ...
        if self.total_wallet_balance < 0:
            raise ValueError(
                f"Negative total wallet balance: {self.total_wallet_balance}"
            )
    ...
    def __str__(self) -> str:
        ...
        return (
            f"AccountInfo(type={self.account_type}, "
...
class ExchangeInfo:
    ...
    def from_binance_response(cls, data: Dict[str, Any]) -> "ExchangeInfo":
        ...
        try:
            ...
            raise ValueError(f"Invalid exchange info format: {e}") from e
...
class SymbolInfo:
    ...
    def from_binance_response(cls, data: Dict[str, Any]) -> "SymbolInfo":
        ...
        try:
            ...
            raise ValueError(f"Invalid symbol info format: {e}") from e
