...
from .config import TradingConfig, get_config, config_manager
...
class SystemInfo:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            self.api_credentials_valid and
            not self.validation_errors and
            len(self.missing_packages) == 0
        )
...
class EnvironmentValidator:
    ...
    def validate_environment(self, config: Optional[TradingConfig] = None) -> SystemInfo:
        """
        ...
        if config is None:
            try:
                ...
                logger.warning("No configuration loaded, proceeding with basic validation")
        ...
        logger.info(f"Environment validation completed. Valid: {self.system_info.is_valid()}")
        return self.system_info
    
    def _validate_python_version(self) -> None:
        """Validate Python version meets requirements."""
        current_version = sys.version_info[:3]
        self.system_info.python_version = f"{current_version[0]}.{current_version[1]}.{current_version[2]}"
    ...
    def _validate_system_info(self) -> None:
        ...
        logger.info(f"✅ System: {self.system_info.platform_system} {self.system_info.platform_release}")
    ...
    def _validate_directory_structure(self) -> None:
        ...
        project_root = Path(self.system_info.project_root) if self.system_info.project_root else Path.cwd()
    ...
    def _validate_required_packages(self) -> None:
        ...
        for package_name, min_version in self.REQUIRED_PACKAGES.items():
            try:
                ...
                self.system_info.required_packages_installed[package_name] = installed_version
    ...
    def _validate_system_resources(self) -> None:
        ...
        try:
            ...
            if hasattr(os, 'sysconf') and os.sysconf('SC_PAGE_SIZE') and os.sysconf('SC_PHYS_PAGES'):
            ...
            if self.system_info.project_root:
                ...
                self.system_info.disk_space_available_gb = (stat.f_bavail * stat.f_frsize) / (1024**3)
            
            # Validate minimum requirements
            if self.system_info.memory_available_gb > 0 and self.system_info.memory_available_gb < 2.0:
                self.system_info.validation_warnings.append(
                    f"Low memory: {self.system_info.memory_available_gb:.1f}GB (recommend 4GB+)"
                )
            
            if self.system_info.disk_space_available_gb > 0 and self.system_info.disk_space_available_gb < 1.0:
                self.system_info.validation_warnings.append(
    ...
    def _validate_configuration(self, config: TradingConfig) -> None:
        ...
        try:
            ...
            self.system_info.validation_errors.append(f"Configuration validation failed: {e}")
            logger.error(f"❌ Configuration validation failed: {e}")
    
    def _validate_api_credentials(self, config: TradingConfig) -> None:
        """Validate API credentials format."""
        self.system_info.api_credentials_valid = config_manager.validate_api_credentials()
        
        if self.system_info.api_credentials_valid:
            logger.info("✅ API credentials format appears valid")
        else:
            self.system_info.validation_errors.append("API credentials are missing or invalid")
    ...
    def _perform_final_validation(self) -> None:
        ...
        critical_errors = [
            error for error in self.system_info.validation_errors
            if any(keyword in error.lower() for keyword in ['python', 'api', 'configuration'])
        ]
        ...
        if total_errors == 0 and total_warnings == 0:
            ...
            logger.info(f"✅ Environment validation passed with {total_warnings} warnings")
        else:
            logger.error(f"❌ Environment validation failed with {total_errors} errors and {total_warnings} warnings")
    ...
    def generate_validation_report(self) -> str:
        ...
        report.append(f"Overall Status: {'✅ VALID' if self.system_info.is_valid() else '❌ INVALID'}")
        report.append("")
        
        # System Information
        report.append("SYSTEM INFORMATION:")
        report.append(f"  Python Version: {self.system_info.python_version}")
        report.append(f"  Platform: {self.system_info.platform_system} {self.system_info.platform_release}")
        ...
        if self.system_info.disk_space_available_gb > 0:
            report.append(f"  Disk Space: {self.system_info.disk_space_available_gb:.1f} GB")
        report.append("")
        ...
        report.append(f"  Environment Valid: {'✅' if self.system_info.environment_valid else '❌'}")
        report.append(f"  API Credentials: {'✅' if self.system_info.api_credentials_valid else '❌'}")
        report.append(f"  Data Directory: {'✅' if self.system_info.data_directory_exists else '⚠️'}")
        report.append(f"  Logs Directory: {'✅' if self.system_info.logs_directory_exists else '⚠️'}")
        report.append("")
