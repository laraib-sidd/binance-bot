...
class DatabaseSchema:
    ...
    async def cleanup_old_data(self, days_to_keep: int = DatabaseSchemaConstants.DEFAULT_RETENTION_DAYS) -> Dict[str, int]:
        ...
        # Compute cutoff date for potential future use in extended cleanup rules
        _ = datetime.now(timezone.utc).replace(
            hour=DatabaseSchemaConstants.CLEANUP_HOUR,
            minute=DatabaseSchemaConstants.CLEANUP_MINUTE,
            second=DatabaseSchemaConstants.CLEANUP_SECOND,
            microsecond=DatabaseSchemaConstants.CLEANUP_MICROSECOND,
        )
