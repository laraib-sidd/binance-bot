...
class TradingLogFormatter(logging.Formatter):
    ...
    def _format_json(self, record: logging.LogRecord) -> str:
        ...
        trading_fields = [
            'trading_pair', 'order_id', 'price', 'quantity', 'side',
            'strategy', 'session_id', 'extra_data'
        ]
        ...
        for field in trading_fields:
            if hasattr(record, field):
                ...
                if value is not None:
                    log_data[field] = str(value) if isinstance(value, Decimal) else value
...
class TradingLoggerAdapter(logging.LoggerAdapter):
    ...
    def trading_action(self, action: str, trading_pair: str, price: Optional[Union[Decimal, float, str]] = None,
                      quantity: Optional[Union[Decimal, float, str]] = None, side: Optional[str] = None,
    ...
    def market_data(self, trading_pair: str, price: Union[Decimal, float, str],
                   volume: Optional[Union[Decimal, float, str]] = None, **kwargs):
    ...
    def signal(self, signal_type: str, trading_pair: str, strength: Optional[float] = None,
              indicators: Optional[Dict[str, Any]] = None, **kwargs):
        """Log trading signal."""
        extra = {
            'trading_pair': trading_pair,
    ...
    def performance(self, metric: str, value: Union[str, float, int], trading_pair: Optional[str] = None,
                   **kwargs):
...
class LoggingManager:
    ...
    def _setup_trading_handler(self):
        ...
        trading_log_file = self.log_directory / f"trading_{datetime.now().strftime('%Y%m%d')}.log"
        ...
        trading_handler.addFilter(lambda record: hasattr(record, 'trading_pair') or 
                                 'trading' in record.name.lower() or
                                 'signal' in record.name.lower() or
    ...
    def _setup_component_loggers(self):
        ...
        components = [
            'helios.core', 'helios.api', 'helios.data', 'helios.strategies',
            'helios.risk', 'helios.backtest', 'helios.signals'
        ]
    ...
    def get_logger(self, name: str, strategy: Optional[str] = None) -> TradingLoggerAdapter:
...
if __name__ == "__main__":
    ...
    logger.signal("BUY_SIGNAL", "BTCUSDT", strength=0.85, indicators={"rsi": 25, "ma_cross": True})
