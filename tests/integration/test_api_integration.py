...
from src.api.exceptions import BinanceAPIError, AuthenticationError
from src.api.models import TickerData, AccountInfo
...
class TestBinanceAPIIntegration:
    ...
    async def test_market_data_retrieval(self, client: BinanceClient):
        ...
        logger.info(f"📊 {test_symbol}: ${ticker.price} ({ticker.price_change_percent_24h:+.2f}%)")
        ...
        for symbol in test_symbols:
            ...
            assert isinstance(tickers[symbol], TickerData), f"{symbol} should be TickerData"
    ...
    async def test_price_data_consistency(self, client: BinanceClient):
        ...
        for symbol in test_symbols:
            ...
            price_diff_percent = abs((current_price - ticker_price) / ticker_price) * 100
            assert price_diff_percent < 5, f"{symbol} price difference too large: {price_diff_percent:.2f}%"
            
            logger.info(f"💱 {symbol}: Current=${current_price}, Ticker=${ticker_price}")
    ...
    async def test_comprehensive_connectivity(self, client: BinanceClient):
        ...
        assert connectivity_result is True, "Comprehensive connectivity test should pass"
            ...
            if success:
                ...
                try:
                    ...
                    if account.balances:
                        ...
                        for asset, balance in list(account.balances.items())[:5]:  # Show first 5
