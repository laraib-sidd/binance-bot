...
class TestConnectionManagers:
    ...
    async def test_r2_connection(self):
        ...
        try:
            ...
            # List objects
            objects = await manager.r2.list_objects(prefix="test/pipeline/", max_keys=10)
    ...
    async def test_connection_manager_lifecycle(self, connection_manager):
        ...
        for service, health in health_status.items():
            assert health.is_healthy, f"{service} is not healthy: {health.error_message}"
            assert health.response_time_ms > 0
...
class TestMarketDataPipeline:
    ...
    async def test_data_quality_monitoring(self, pipeline):
        ...
        poor_quality_ticker = TickerData(
            ...
            timestamp=datetime.now(timezone.utc) - timedelta(minutes=10)  # Old timestamp
        )
    ...
    async def test_get_current_price_fallback(self, pipeline):
        ...
        await pipeline.connection_manager.redis.set("price:FALLBACKUSDT", "456.78", ex=300)
    ...
    async def test_pipeline_metrics(self, pipeline):
        ...
        initial_total = initial_health['metrics']['total_updates']
        
        # Process some test data
        test_ticker = TickerData(
            symbol='METRICSUSDT',
            price=Decimal('1.234'),
            bid_price=Decimal('1.233'),
            ask_price=Decimal('1.235'),
            volume_24h=Decimal('1000.0'),
            price_change_24h=Decimal('0.01'),
            price_change_percent_24h=Decimal('0.81'),
            high_24h=Decimal('1.25'),
            low_24h=Decimal('1.22'),
            timestamp=datetime.now(timezone.utc)
        )
        
        await pipeline._process_single_ticker('METRICSUSDT', test_ticker)
...
async def cleanup_test_data():
    ...
    try:
        ...
        test_symbols = ['BTCUSDT', 'TESTUSDT', 'FALLBACKUSDT', 'METRICSUSDT', 'VALIDUSDT', 'INVALID']
        
        for symbol in test_symbols:
            await manager.postgres.execute(
                "DELETE FROM current_prices WHERE symbol = $1", 
                symbol
            )
            await manager.postgres.execute(
                "DELETE FROM data_quality_metrics WHERE symbol = $1", 
                symbol
            )
