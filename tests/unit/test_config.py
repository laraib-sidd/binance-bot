...
from src.core.config import (
    TradingConfig, ConfigurationManager, load_configuration,
    validate_configuration, get_config, config_manager
)
...
class TestTradingConfig:
    ...
    def test_risk_parameter_validation(self):
        ...
        with pytest.raises(ValueError, match="max_account_drawdown_percent must be between 0 and 100"):
            TradingConfig(
                binance_api_key="a" * 64,
                binance_api_secret="b" * 64,
    ...
    def test_trading_pairs_validation(self):
        ...
        with pytest.raises(ValueError, match="At least one trading pair must be specified"):
            TradingConfig(
                binance_api_key="a" * 64,
                binance_api_secret="b" * 64,
...
LOG_LEVEL=INFO
    ...
    def test_load_from_nonexistent_env_file(self):
        """Test loading from non-existent .env file raises error."""
        manager = ConfigurationManager()
        
        with pytest.raises(FileNotFoundError):
            manager.load_from_env_file("nonexistent.env")
    ...
    def test_validate_api_credentials(self):
        ...
        assert manager.validate_api_credentials() is False
        ...
        with patch.dict(os.environ, test_env_invalid, clear=False):
            # This should raise an exception during config creation
    ...
    def test_get_safe_config_summary(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            ...
            assert summary["api_keys_configured"] is True
            assert "binance_api_key" not in summary
